...
void THttpClient::flush() {
  ...
  std::ostringstream h;
  h <<
    "POST " << path_ << " HTTP/1.1" << HTTP_CRLF <<
    "Host: " << host_ << HTTP_CRLF <<
    "Content-Type: application/x-thrift" << HTTP_CRLF <<
    "Content-Length: " << len << HTTP_CRLF <<
    "Accept: application/x-thrift" << HTTP_CRLF <<
    "User-Agent: Thrift/" << VERSION << " (C++/THttpClient)" << HTTP_CRLF <<
    HTTP_CRLF;
  string header = h.str();
