...
const char* THttpTransport::HTTP_CRLF = "\r\n";
const int THttpTransport::HTTP_CRLF_LEN = 2;
...
uint32_t THttpTransport::readChunked() {
  ...
  if (chunkSize == 0) {
    ...
    // Read trailing HTTP_CRLF after content
    readLine();
  }
  return length;
}
...
char* THttpTransport::readLine() {
  while (true) {
    ...
    eol = strstr(httpBuf_+httpPos_, HTTP_CRLF);
    ...
    if (eol == NULL) {
      ...
      *eol = '\0';
      ...
      httpPos_ = static_cast<uint32_t>((eol-httpBuf_) + HTTP_CRLF_LEN);
      return line;
    }
  }
