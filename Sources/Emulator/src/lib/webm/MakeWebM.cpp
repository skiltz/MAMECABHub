...
int stats_open_mem(stats_io_t *stats, int pass)
{
    ...
    if (!pass)
    {
        ...
        stats->buf.buf = (char*)malloc(stats->buf_alloc_sz);
    }
                                ...
                                const vpx_rational_t      *fps)
{
    ...
    hist->pts = (int64_t*)calloc(hist->samples, sizeof(*hist->pts));
    hist->sz = (int*)calloc(hist->samples, sizeof(*hist->sz));
...
int main_unused(int argc, const char **argv_)
{
...
#define RGB_TO_YUV(R,G,B,Y,U,V) Y = (R)/3 + (G)/3 + (B)/3; U = (( (3*((int)(B))) - (3*(((int)(Y)))) + 512 )>>2); V = (( (3*((int)(R))) - (3*(((int)(Y)))) + 512 )>>2);
		    ...
		    //printf("IMAGE INFO: %d %d %d %d %d %d %d\n",raw.stride[0],raw.stride[1],raw.stride[2],raw.stride[3],raw.bps,raw.w,raw.h);
		    {
		      unsigned char *chr = raw.planes[0];
		      for(i=0;i<raw.h;i++) {
			for(j=0;j<raw.w;j++) {
			  unsigned char dummy1,dummy2;
			  RGB_TO_YUV(0,0,j,chr[i*raw.w+j],dummy1,dummy2);
			  //printf("Y = %d U = %d V = %d\n",chr[i*raw.w+j],dummy1,dummy2);
			}
		      }
		    }
		    {
			...
			for(j=0;j<raw.w/2;j++) {
			  unsigned char dummy1,dummy2;
			  RGB_TO_YUV(0,0,j*2,dummy1,chr[i*raw.w/2+j],dummy2);
			  //chr[i*raw.w/2+j] = 128;
			  //printf("R,G,B = %d Y = %d U = %d V = %d\n",j*2,dummy1,chr[i*raw.w/2+j],dummy2);
			}
		      }
		    }
		    {
			...
			for(j=0;j<raw.w/2;j++) {
			  unsigned char dummy1,dummy2;
			  RGB_TO_YUV(0,0,j*2,dummy1,dummy2,chr[i*raw.w/2+j]);
			  //chr[i*raw.w/2+j] = 128;
			  //printf("R,G,B = %d Y = %d U = %d V = %d\n",j*2,dummy1,dummy2,chr[i*raw.w/2+j]);
			}
		      }
		    }
		}
	    ...
	    //printf("FRAME START %lld NEXT FRAME START %lld\n",frame_start,next_frame_start);
        ...
        fprintf(stderr,
               ...
               1, frames_in, frames_out, nbytes, nbytes * 8 / frames_in,
