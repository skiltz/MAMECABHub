...
StartupResult RakPeer::Startup( unsigned short maxConnections, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount, int threadPriority )
{
	...
	for (i=0; i<socketDescriptorCount; i++)
	{
		...
		// Test the socket
		int zero=0;
		if (SocketLayer::SendTo((SOCKET)rns->s, (const char*) &zero,4,"127.0.0.1", rns->boundAddress.port, rns->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_)!=0)
		{
			DerefAllSockets();
...
//	SystemAddress systemAddress;
	...
	{
		if (SocketLayer::SendTo(socketList[i]->s, (const char*) &i,1,"127.0.0.1", socketList[i]->boundAddress.port, socketList[i]->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_)!=0)
			break;
	}
...
//	unsigned int socketIndex;
	...
	while ( isRecvFromLoopThreadActive && RakNet::GetTimeMS()<timeout )
	{
		...
		for (i=0; i < socketList.Size(); i++)
		{
			SocketLayer::SendTo(socketList[i]->s, (const char*) &i,1,"127.0.0.1", socketList[i]->boundAddress.port, socketList[i]->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_);
		}
...
//		return;
	...
	SocketLayer::SendTo( socketList[realIndex]->s, (const char*)bitStream.GetData(), (int) bitStream.GetNumberOfBytesUsed(), ( char* ) host, remotePort, socketList[realIndex]->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_ );
...
int RakPeer::GetHighestPing( const AddressOrGUID systemIdentifier )
{
	int maxPing=0;
	int quantity;
	RemoteSystemStruct *remoteSystem = GetRemoteSystem( systemIdentifier, false, false );
	...
	for ( quantity = 0; quantity < PING_TIMES_ARRAY_SIZE; quantity++ )
	{
        ...
        {
			maxPing = max(maxPing, int(remoteSystem->pingAndClockDifferential[ quantity ].pingTime) );
        }
	}
...
bool RakPeer::GenerateConnectionRequestChallenge(RequestedConnectionStruct *rcs,PublicKey *publicKey)
{
	...
	case PKM_USE_TWO_WAY_AUTHENTICATION:
		...
		{
			//printf("AUDIT: Failure initializing new client_handshake object with identity for this RequestedConnectionStruct\n");
			RakNet::OP_DELETE(rcs->client_handshake,_FILE_AND_LINE_);
			rcs->client_handshake=0;
			return false;
		}

		//printf("AUDIT: Success initializing new client handshake object with identity for this RequestedConnectionStruct -- pre-generated challenge\n");
...
bool ProcessOfflineNetworkPacket( const SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer, RakNetSmartPtr<RakNetSocket> rakNetSocket, bool *isOfflineMessage, RakNet::TimeUS timeRead )
{
	...
	if (rakPeer->IsBanned( str1 ))
	{
		...
		SocketLayer::SendTo( rakNetSocket->s, (char*) bs.GetData(), bs.GetNumberOfBytesUsed(), systemAddress.binaryAddress, systemAddress.port, rakNetSocket->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_ );
...
// #if !defined(_XBOX) && !defined(X360)
				...
				char str1[64];
				systemAddress.ToString(false, str1);
				SocketLayer::SendTo( rakNetSocket->s, (const char*)outBitStream.GetData(), (unsigned int) outBitStream.GetNumberOfBytesUsed(), str1 , systemAddress.port, rakNetSocket->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_ );
...
// #endif
		...
		else if ((unsigned char)(data)[0] == (MessageID)ID_OPEN_CONNECTION_REPLY_1)
		{
			...
			for (i=0; i <  rakPeer->requestedConnectionQueue.Size(); i++)
			{
				...
				if (rcs->systemAddress==systemAddress)
				{
					...
					SocketLayer::SendTo( rakPeer->socketList[rcs->socketIndex]->s, (const char*) bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), rcs->systemAddress.binaryAddress, rcs->systemAddress.port, rakPeer->socketList[rcs->socketIndex]->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_ );
		...
		{/*
			...
			SocketLayer::SendTo( rakNetSocket->s, (const char*) bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), systemAddress.binaryAddress, systemAddress.port, rakNetSocket->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_ );
		}
...
/*					printf("AUDIT: RECV CHALLENGE ");
			...
			if (outcome==1)
			{
				...
				SocketLayer::SendTo( rakNetSocket->s, (const char*) bsAnswer.GetData(), bsAnswer.GetNumberOfBytesUsed(), systemAddress.binaryAddress, systemAddress.port, rakNetSocket->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_ );
			...
			if (rcs->nextRequestTime < timeMS)
			{
				...
				else
				{
					...
					if (rcs->socket.IsNull())
					{
						...
						if (SocketLayer::SendTo( socketList[rcs->socketIndex]->s, (const char*) bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), rcs->systemAddress.binaryAddress, rcs->systemAddress.port, socketList[rcs->socketIndex]->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_ )==-10040)
						{
					...
					else
					{
						...
						RakNet::Time sendToStart=RakNet::GetTime();
						if (SocketLayer::SendTo( rcs->socket->s, (const char*) bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), rcs->systemAddress.binaryAddress, rcs->systemAddress.port, socketList[rcs->socketIndex]->remotePortRakNetWasStartedOn_PS3, _FILE_AND_LINE_  )==-10040)
						{
							// Don't use this MTU size again
							rcs->requestsMade = (unsigned char) ((MTUSizeIndex + 1) * (rcs->sendConnectionAttemptCount/NUM_MTU_SIZES));
							rcs->nextRequestTime=timeMS;
